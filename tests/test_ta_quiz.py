...
def test_start_quiz_refuses_when_already_active():
    ...
    with (
        patch("bot.ta.quiz.get_active_quiz", return_value=existing),
...
# ── record_answer ─────────────────────────────────────────────────────────
# Answers now go through record_quiz_answer (one HSET per student) instead
# of read-modify-write on the active-quiz dict. The previous shared-dict
# write could drop one student's answer when two replied at the same tick.
def test_record_answer_stores_and_reacts():
    ...
    with (
        patch("bot.ta.quiz.get_active_quiz", return_value=existing),
        patch("bot.ta.quiz.record_quiz_answer") as rec,
        patch("bot.ta.quiz.set_reaction") as react,
    ):
        from bot.ta.quiz import record_answer

        record_answer(_prepared(text="A"), "A")
        # One atomic write per student, keyed by chat_id + user_id.
        rec.assert_called_once()
        chat_id, user_id, payload = rec.call_args.args
        assert chat_id == -100123
        assert user_id == 42
        assert payload["letter"] == "A"
...
def test_record_answer_overwrites_previous():
    ...
    with (
        ...
        # HSET overwrites by Redis semantics — the test just confirms the
        # latest letter is what gets persisted.
        payload = rec.call_args.args[2]
...
def test_inline_reveal_skips_fresh_quiz():
    ...
    with (
        patch("bot.ta.quiz.get_active_quiz", return_value=fresh),
