...
def _matches():
    return [
        {
            "title": "CS101",
            "chunkText": "Python is a language.",
            "blobUrl": "https://b/a",
            "score": 0.9,
        },
        {
            "title": "CS101",
            "chunkText": "More on Python.",
            "blobUrl": "https://b/a",
            "score": 0.8,
        },
        {
            "title": "NumPy",
            "chunkText": "Arrays are fast.",
            "blobUrl": "https://b/b",
            "score": 0.7,
        },
    ]
...
def test_answer_cites_only_sources_listed_in_trailer():
    with (
        patch("bot.ai.rag.retrieve", return_value=_matches()),
        patch("bot.ai.get_history", return_value=[]),
    ...
    matches = [
        {
            "title": "Phantom",
            "chunkText": "",
            "blobUrl": "https://b/phantom",
            "score": 0.9,
        },
        {
            "title": "Real",
            "chunkText": "real text",
            "blobUrl": "https://b/real",
            "score": 0.8,
        },
    ]
    with (
        patch("bot.ai.rag.retrieve", return_value=matches),
        patch("bot.ai.get_history", return_value=[]),
        patch("bot.ai.append_history"),
        patch("bot.ai.get_active_model", return_value=None),
        patch("bot.ai.ai") as client,
...
def test_answer_caps_citations_at_five():
    ...
    with (
        patch("bot.ai.rag.retrieve", return_value=six),
        patch("bot.ai.get_history", return_value=[]),
        patch("bot.ai.append_history"),
        patch("bot.ai.get_active_model", return_value=None),
        patch("bot.ai.ai") as client,
...
def test_answer_cites_match_without_blob_url_by_title():
    matches = [
        {"title": "Local Notes", "chunkText": "notes", "blobUrl": "", "score": 0.9}
    ]
    with (
        patch("bot.ai.rag.retrieve", return_value=matches),
        patch("bot.ai.get_history", return_value=[]),
...
def test_answer_whitespace_only_title_in_citation_falls_back_to_doc():
    matches = [
        {"title": "   ", "chunkText": "x", "blobUrl": "https://b/x", "score": 0.9}
    ]
    with (
        patch("bot.ai.rag.retrieve", return_value=matches),
        patch("bot.ai.get_history", return_value=[]),
        ...
        assert "[   ]" not in out
        # Falls back to literal "doc" as the link text when the title is
        # blank/whitespace. Rendered as an HTML anchor since send_reply uses
        # parse_mode="HTML".
        assert ">doc</a>" in out
