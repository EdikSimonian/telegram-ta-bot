...
def test_trim_leading_reasoning_removes_okay_preamble():
    ...
    text = "Okay, the user is asking about Python.\n\nPython is a programming language."
...
def test_is_hedging_false_for_long_answer_that_opens_with_caveat():
    from bot.ta.guardrail import is_hedging

    text = (
        "I don't have information about your exact bot, but generally you "
        "write a test by building a fake Telegram update, calling route(msg), "
        "and asserting that send_message was called with the expected text."
    )
    assert is_hedging(text) is False


# ── inappropriate-content (teen safety) — whole-word, not substring ────────
def test_is_inappropriate_content_whole_word_only():
    from bot.ta.guardrail import is_inappropriate_content

    # Confirmed prod false positive: benign answer with "whatever" -> "hate".
    assert is_inappropriate_content("Whatever the command returns, assert it.") is False
    assert is_inappropriate_content("It's like a postal code in Sussex.") is False
    # Genuine whole-word hits still flagged:
    assert is_inappropriate_content("This involves weapons and violence.") is True


def test_clean_passes_benign_answer_containing_hate_substring():
    from bot.ta.guardrail import clean

    text = "To test a bot command, call route(); whatever it returns, assert on it."
...
def test_ai_answer_suppresses_hedged_reply():
    ...
    with (
        ...
        client.chat.completions.create.return_value = MagicMock(
            choices=[
                MagicMock(message=MagicMock(content="I don't have access to that."))
            ]
        )
        from bot.ai import answer

        p = MagicMock(
            stripped_text="when is class",
            group_key="-100123",
            is_dm=False,
            is_mention=False,
