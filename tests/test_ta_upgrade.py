...
def test_fire_requires_config():
    ...
    with (
        patch("bot.ta.upgrade.CLAUDE_ROUTINE_ID", ""),
...
def test_fire_rejects_empty_instructions():
    ...
    with (
        patch("bot.ta.upgrade.CLAUDE_ROUTINE_ID", "rt_abc"),
...
def test_fire_wraps_network_errors():
    ...
    with (
        ...
        patch("bot.ta.upgrade.CLAUDE_ROUTINE_TOKEN", "sk-ant-oat01-xyz"),
        patch(
            "bot.ta.upgrade.requests.post", side_effect=requests.ConnectionError("boom")
        ),
    ):
...
def test_upgrade_fires_routine_and_replies_with_session_link():
    ...
    with (
        ...
        # The instruction is wrapped with the branch-workflow directive (push
        # to test -> auto-deploy, PR test -> main) before firing.
        fire.assert_called_once()
        fired = fire.call_args.args[0]
        assert fired.endswith("add a /ping command")
        assert "`test`" in fired and "`main`" in fired
        assert "pull request" in fired.lower()
        text = sm.call_args.args[1]
...
def test_upgrade_surfaces_fire_error_to_admin():
    ...
    with (
        patch("bot.ta.commands.send_message") as sm,
        patch(
            "bot.ta.commands.upgrade_mod.fire",
